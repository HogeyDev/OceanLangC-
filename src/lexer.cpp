...
Token* Lexer::nextToken() {
    ...
    default: {
        ...
        std::cerr << "\tAt Location " << this->line + 1 << ":" << this->column + 1 << std::endl;
        exit(1);
    }
    }
...
Token* Lexer::lexInt() {
    ...
    Token* ret = new Token(integer, TokenType::INTLIT);
    return ret;
}
...
std::vector<Token*> Lexer::getTokenList() {
    ...
    Token* currentToken;
    while ((currentToken = this->nextToken())->type != TokenType::ENDOFFILE)
