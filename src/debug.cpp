...
std::string getPrintableFunctionDeclaration(AST::FunctionDeclaration* ast, int tabs) {
...
std::string getPrintableIntegerLiteral(AST::IntegerLiteral* ast, int tabs) {
    return calculateTabs(tabs) + "IntegerLiteral(value: " + std::to_string(ast->value) + ")";
}
...
std::string getPrintableTerm(AST::Term* ast, int tabs) {
    ...
    struct TermVisitor {
        std::string& ret;
        int tabs;
        void operator()(AST::IntegerLiteral* ast) { ret += getPrintableIntegerLiteral(ast, tabs); }
        void operator()(AST::Identifier* ast) { ret += getPrintableIdentifier(ast, tabs); }
        void operator()(AST::StringLiteral* ast) { ret += getPrintableStringLiteral(ast, tabs); }
        void operator()(AST::Expression* ast) { ret += getPrintableExpression(ast, tabs); }
        void operator()(AST::VariableRecall* ast) { ret += getPrintableVariableRecall(ast, tabs); }
    };
...
std::string getPrintableBinaryExpression(AST::BinaryExpression* ast, int tabs) { return "BINARYEXPR"; }
...
std::string getPrintableExpression(AST::Expression* ast, int tabs) {
    ...
    struct ExpressionVisitor {
        ...
        void operator()(AST::BinaryExpression* ast) { ret += getPrintableBinaryExpression(ast, tabs); }
    };
...
std::string getPrintableVariableAssignment(AST::VariableAssignment* ast, int tabs) {
    return calculateTabs(tabs) + "VariableAssignment(variableName:\n" + calculateTabs(tabs + 1) +
           getPrintableIdentifier(ast->variableName) + "\n" + calculateTabs(tabs + 1) +
           "type: " + getPrintablePrimType(ast->type) + "\n" + calculateTabs(tabs + 1) + "value:\n" +
           getPrintableExpression(ast->value, tabs + 2) + "\n" + calculateTabs(tabs) + ")";
}

std::string getPrintableArgument(AST::Expression* ast, int tabs) { return getPrintableExpression(ast, tabs); }
...
std::string getPrintableStatement(AST::Statement* ast, int tabs) {
    std::string ret;
    // std::variant<VariableAssignment*, FunctionDeclaration*, FunctionCall*, Import*>

    struct StatementVisitor {
        std::string& ret;
        int tabs;
        void operator()(AST::VariableAssignment* ast) { ret += getPrintableVariableAssignment(ast, tabs); }
        void operator()(AST::FunctionDeclaration* ast) { ret += getPrintableFunctionDeclaration(ast, tabs); }
        void operator()(AST::FunctionCall* ast) { ret += getPrintableFunctionCall(ast, tabs); }
        void operator()(AST::Import* ast) { ret += getPrintableImport(ast, tabs); }
    };
